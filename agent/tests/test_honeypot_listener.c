 ...
 *   - JSON contains "src_ip":"10.0.1.87"
...
static void test_build_probe_event_json(void)
{
    ...
    /* Peer address: 10.0.1.87:54321 */
    memset(&session.peer, 0, sizeof(session.peer));
    session.peer.sin_family = AF_INET;
    inet_pton(AF_INET, "10.0.1.87", &session.peer.sin_addr);
    ...
    if (m.count >= 1) {
        ...
        CHECK(strstr(js, "\"src_ip\":\"10.0.1.87\"") != NULL ||
              strstr(js, "\"src_ip\": \"10.0.1.87\"") != NULL,
              "probe_event: src_ip is 10.0.1.87");
...
static void test_halfopen_syn_packet(void)
{
    ...
    pkt[26] = 10; pkt[27] = 0; pkt[28] = 1; pkt[29] = 87;    /* src 10.0.1.87 */
