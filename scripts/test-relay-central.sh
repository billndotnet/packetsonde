#!/bin/bash
# Two-agent relay round-trip against central (loopback). ASSISTED: the relay needs a
...
EXP="PYTHONPATH=/opt/repo/rna/rna BASE_CONFIG=/opt/etc/central/config.yml"
...
EOF
  ...
  env $EXP $PY -c "from backend.framework.es import get_es_client; get_es_client().update(index='central_ps_agents', id='$id', doc={'status':'validated'}, refresh='wait_for')"
...
=== VERIFY (central) ===
  - central_ps_events has the $EDGE event: transport=relay, relay_path[0].verified=true,
    relay_chain_verified=true, relay_path[0].relay_agent_id=$RELAY
  - central_ps_relay_edges has ($EDGE -> $RELAY) state=verified

=== CLEANUP ===
  env $EXP $PY -c "from backend.framework.es import get_es_client as g; es=g(); \\
    [es.options(ignore_status=[404]).delete(index='central_ps_agents', id=i) for i in ('$EDGE','$RELAY')]"
  (also delete the $EDGE/$RELAY docs from central_ps_events + central_ps_relay_edges)
  rm -rf $TMP
EOF
