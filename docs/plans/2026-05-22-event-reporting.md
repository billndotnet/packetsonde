...
Expected: `PARITY OK: C signature verified by Python over raw payload bytes`. (Requires `python3` with the `cryptography` package — present in the rna venvs; use `/opt/psdev-0522-2/venv/bin/python` if the system python lacks it.)
...
## Task 8: Live integration against psdev
...
# validate it (set status=validated in psdev_ps_agents)
PYTHONPATH=/opt/repo/rna/rna BASE_CONFIG=/opt/etc/psdev/config.yml /opt/psdev-0522-2/venv/bin/python -c "
from backend.framework.es import get_es_client
es=get_es_client(); es.update(index='psdev_ps_agents', id='$AID', doc={'status':'validated'}, refresh='wait_for')
print('validated $AID')"
...
PYTHONPATH=/opt/repo/rna/rna BASE_CONFIG=/opt/etc/psdev/config.yml /opt/psdev-0522-2/venv/bin/python -c "
from backend.framework.es import get_es_client
es=get_es_client(); es.indices.refresh(index='psdev_ps_events')
h=es.search(index='psdev_ps_events', query={'term':{'agent_id':'$AID'}})['hits']['hits']
print('events for $AID:', len(h), h[0]['_source'].get('kind') if h else '-')
for i in ('psdev_ps_events',): pass
es.options(ignore_status=[404]).delete(index='psdev_ps_agents', id='$AID')
import sys
for d in h: es.options(ignore_status=[404]).delete(index='psdev_ps_events', id=d['_id'])
print('cleaned up')"
```
