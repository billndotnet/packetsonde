...
> Transport is exercised end-to-end in Task 8's integration test against live central.
...
static void test_payload(void) {
    ...
    struct ps_reg_input in = { "edge-07", pub, "sha256:abc", "host", "direct", "192.0.2.42" };
    char buf[1024];
    ...
    assert(strstr(buf, "\"ip_address\":\"192.0.2.42\""));
...
## Task 8: Integration test — register against live central

**Files:**
- Create: `scripts/test-register-central.sh`

- [ ] **Step 1: Create `scripts/test-register-central.sh`**

```bash
#!/bin/bash
# Integration: register a throwaway agent against the live central central,
# verify a pending ps_agents doc appears, then clean up.
# Usage: scripts/test-register-central.sh http://192.0.2.10:8700
set -e
...
echo "registered; verify in central: GET /api/v1/packetsonde/agents?status=pending should include $AID"
echo "agent_id=$AID  (clean up the pending ps_agents doc afterward)"
rm -rf "$TMP"
```

- [ ] **Step 2: Run it (manual, requires reachable central)**

Run: `chmod +x scripts/test-register-central.sh && ./scripts/test-register-central.sh http://192.0.2.10:8700`
Expected: prints "registered (pending validation)"; the `$AID` then appears in central's admin Agents list as `pending`. (Validate it → run a checkin from the same config to confirm it shows online.)

- [ ] **Step 3: Commit**

```bash
git add scripts/test-register-central.sh
git commit -m "Add central registration integration smoke-test script"
```
...
- §8 testing (unit + live-central integration) → Tasks 1,3,5 (unit) + 8 (integration) ✓
