...
## Task 8: Live two-agent relay round-trip (psdev)

**Files:** Create `scripts/test-relay-psdev.sh`
...
```bash
#!/bin/bash
# Two-agent relay round-trip against psdev (loopback). Requires the agent built
# and psdev running the merged central. Uses uuid-scoped ids + cleans up.
set -e
cd "$(dirname "$0")/.."
PY=/opt/psdev-0522-3/venv/bin/python
EXPORTS="PYTHONPATH=/opt/repo/rna/rna BASE_CONFIG=/opt/etc/psdev/config.yml"
...
EOF
  ...
  env $EXPORTS $PY -c "from backend.framework.es import get_es_client; get_es_client().update(index='psdev_ps_agents', id='$id', doc={'status':'validated'}, refresh='wait_for')"
done
...
echo "Verify: psdev_ps_events has the edge event transport=relay, relay_path[0].verified=true,"
echo "relay_chain_verified=true; psdev_ps_relay_edges has ($EDGE -> $RELAY) state=verified."
echo "Cleanup: delete test agents/events/edges for $EDGE/$RELAY."
```

- [ ] **Step 2: Run the assisted live test** (the daemon listener needs root/caps; drive it per the script's printed steps, or via systemd on the host). Confirm in `psdev_ps_events`: `transport=relay`, `relay_path[0].verified=true`, `relay_chain_verified=true`; and `psdev_ps_relay_edges` shows `(edge→relay) verified`. Clean up the test docs.

- [ ] **Step 3: Commit**

```bash
cd /opt/repo/packetsonde
git add scripts/test-relay-psdev.sh
