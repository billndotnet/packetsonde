...
**Supersedes:** This document narrows the project's near-term scope away from the UE5 3D frontend (frozen, not deleted) and toward a CLI-first network infrastructure and security auditing tool suite. The whitepaper at `docs/specs/whitepaper.md` remains the long-term vision; this spec governs the work between now and when the visualizer is reactivated.
...
The current `agent/` directory is renamed `src/agent/` and joined by `src/cli/` and `src/lib/`. The UE5 frontend (`Content/`, `Source/`) is left untouched.
...
├── docs/
├── Content/, Source/   # UE5 stuff — frozen, untouched
...
- The UE5 3D frontend is **frozen, not deleted**. No CLI changes touch `Content/` or `Source/`. The whitepaper's visualization roadmap is paused.
