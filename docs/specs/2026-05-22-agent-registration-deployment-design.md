...
```toml
[central]
# Central rna-packetsonde base URL. http:// for dev (e.g. psdev :8700),
...
**Integration (against live psdev central):**
- `packetsonde register --provenance direct --config <test cfg pointing at psdev>` →
  assert `201`, then query psdev `ps_agents` for the `pending` record (correct pubkey,
