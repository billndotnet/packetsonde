...
**Live (central, two-agent loopback):**
...
- Assert: the event lands in `central_ps_events` with `transport="relay"`, a `relay_path`
  whose single hop is `verified:true`, `relay_chain_verified:true`; and
  `central_ps_relay_edges` shows the `edge→relay` edge in state `verified`. UUID-scoped ids
