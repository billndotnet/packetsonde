...
PORT="${PS_TEST_MC_PORT:-$((30000 + RANDOM % 20000))}"
...
OUT="$("$CLI" --jsonl audit memcached "127.0.0.1:$PORT" 2>/dev/null || true)"
fail() { echo "FAIL: $1"; echo "OUTPUT:"; echo "$OUT"; exit 1; }
echo "$OUT" | grep -q '"kind":"memcached.metadata"'         || fail "missing memcached.metadata"
echo "$OUT" | grep -q '"kind":"memcached.noauth_exposed"'   || fail "missing memcached.noauth_exposed"
echo "$OUT" | grep -q '1.6.17'                              || fail "missing version"
echo "test_audit_memcached: OK"
