...
### Added
- **JA3 / JA3S fingerprints in `audit tls`** — captures the raw `ClientHello` and `ServerHello` via `SSL_CTX_set_msg_callback`, computes the JA3 (our client) and JA3S (the server) fingerprint strings + MD5s, and adds `ja3` / `ja3_str` / `ja3s` / `ja3s_str` to `tls.metadata` evidence. GREASE values (RFC 8701) are skipped so fingerprints stay stable across probes. Defenders can match on `ja3` to recognise packetsonde traffic; analysts get `ja3s` as a server-stack fingerprint useful for grouping hosts behind load balancers.
